<template>
  <component
    :is="column.search?.render ?? componentMap[column.search?.el ?? ''] ?? `el-${column.search?.el}`"
...
import ZyfDtYearPicker from "../../ZyfDtYearPicker/index.vue";

const componentMap: { [key: string]: any } = {
  "year-picker": ZyfDtYearPicker
};
...
const placeholder = computed(() => {
  ...
  const placeholder =
    ...
    `${search?.el?.includes("input") ? "请输入" : "请选择"}${search?.label ?? props.column?.label ?? ""}`;
  return { placeholder };
});
